...
class OTPActivity : AppCompatActivity() {
		...
		auth.signInWithCredential(credential)
			.addOnCompleteListener(this) { task ->
				if (task.isSuccessful) {
					...
					val userModel = UserModel(user!!.uid, phoneNumber, "0.0")
	...
	fun saveUserInDatabase(userModel: UserModel) {
		...
		val databaseReference =
			firebaseDatabase.getReference("users").child(phoneNumber).setValue(userModel)
				.addOnCompleteListener { task ->
					if (task.isSuccessful) {
						Log.d(TAG, "saveUserInDatabase: ${task.result}")
					}
				}
	}
